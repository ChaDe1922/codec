...
fftmax = max(abs(fft(sig.*win))); % defined as 96dB  %Normalization
...
for frame_count=1:length(frames(:,1))
    ...
    if fft_frame == zeros(1,N)
        ...
        fft_spl = 96 + 20*log10(abs(fft_frame)/fftmax);  %highest possible value should be 0dB  %fft SPL Normalization
...
for ii=1:N/2
    num_crit_band_samples(crit_band(ii)) = num_crit_band_samples(crit_band(ii)) + 1;    %%Spend dome time here
end
